...
function _page($$renderer) {
  const project = {
    ...
    summary: "I translated Peter Shirley's CPU implementation into an HLSL compute shader that runs inside Unity. The build focuses on the fundamentals: sampling BRDFs, bouncing rays, denoising, and exposing everything through lightweight editor tooling.",
    stack: ["Unity", "HLSL", "Compute Shaders", "C#"],
    duration: "3 weeks of evenings",
    role: "Graphics programmer",
    team: "Solo project",
    contribution: "Recreated the complete renderer on the GPU, built buffer packing utilities in C#, and exposed live controls for bounces, samples per pixel, and depth of field to study how each lever affects convergence."
  };
  const highlights = [
    {
      label: "Goal",
  ...
  const galleryImages = [
    {
      alt: "First converged diffuse render from the compute shader",
      src: raytracerShot
    },
    {
      alt: "Metals, dielectrics, and emissive spheres with depth of field",
      src: raytracerShot
    },
    {
      alt: "Debug view showing surface normals and focus plane",
      src: raytracerShot
    }
  ];
  $$renderer.push(`<main class="min-h-screen bg-[#edf2ff] text-slate-900"><section class="bg-gradient-to-br from-slate-950 via-blue-900 to-slate-800 text-white"><div class="max-w-6xl mx-auto px-6 py-16 lg:py-24 flex flex-col gap-12 lg:flex-row lg:items-center"><div class="space-y-6 max-w-xl"><h1 class="text-4xl font-semibold lg:text-5xl">${escape_html(project.title)}</h1> <p class="text-xl text-white/85">${escape_html(project.tagline)}</p> <p class="text-white/80 leading-relaxed">${escape_html(project.summary)}</p> <div class="flex flex-wrap gap-2"><!--[-->`);
  ...
  $$renderer.push(`<!--]--></div></div></section> <section class="max-w-5xl mx-auto px-6 py-16 space-y-12"><div class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]"><article class="rounded-3xl bg-white p-8 shadow-[0_20px_60px_rgba(79,70,229,0.2)]"><h3 class="text-xl font-semibold text-gray-900">Experience beats</h3> <ul class="mt-4 space-y-4 text-gray-600"><li>- Progressive accumulation slider to study noise vs. performance.</li> <li>- Camera controls for aperture, focus distance, and shutter time to test DOF + motion blur.</li> <li>- Debug overlays that display normals, ray depth, or albedo to quickly find sampling bugs.</li></ul></article> <article class="rounded-3xl border border-gray-200 bg-white p-8"><h3 class="text-xl font-semibold text-gray-900">Compute shader pipeline</h3> <p class="mt-4 text-gray-600">Rays are launched in tiles to keep thread groups coherent. Each bounce samples the material, updates energy, and decides whether to terminate via
          Russian roulette. The C# side streams scene buffers (spheres, materials, camera) every edit while async GPU readback saves converged frames.</p></article></div> <div class="grid gap-6 md:grid-cols-3"><!--[-->`);
  ...
  $$renderer.push(`<!--]--></div></div></section> <section class="max-w-5xl mx-auto px-6 pb-16"><div class="rounded-[32px] bg-slate-900 text-white p-10 flex flex-col gap-6 md:flex-row md:items-center md:justify-between"><div><p class="text-sm uppercase tracking-[0.4em] text-gray-400">Next</p> <h2 class="text-3xl font-semibold">Continuing the series</h2> <p class="mt-3 text-gray-300">I am currently diving into the sequels from the Ray Tracing in One Weekend series (BVHs, textures, quads). If you are exploring similar graphics experiments, let's chat.</p></div> <div class="flex flex-col gap-3 md:items-end"><a href="/contact" class="cta cta-primary">Start a graphics convo</a> <a href="/Projects/ProjectView" class="cta cta-ghost text-white border-white/40">Browse more projects</a></div></div></section></main>`);
}
export {
  _page as default
};
